...
export default function SearchStudent() {
  ...
  const statusOptions = [
    ...
    { value: 'has_sessions', label: 'Sessions Left' },
    { value: 'no_sessions', label: 'No Sessions Left' },
  ]
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (selectedYear !== 'All') {
      result = result.filter(s => String(s.year_level) === selectedYear)
    }

    if (selectedStatus !== 'All') {
      result = result.filter(s => {
        const hasActiveSession = activeSessionIds.has(String(s.id_number || ''))
        if (selectedStatus === 'in_session') return hasActiveSession
        if (selectedStatus === 'has_sessions') return !hasActiveSession && Number(s.available_sessions) > 0
        if (selectedStatus === 'no_sessions') return !hasActiveSession && Number(s.available_sessions) <= 0
        return true
      })
    }

    return result
  }, [students, keyword, selectedCourse, selectedYear, selectedStatus, activeSessionIds])
  ...
  useEffect(() => {
    setPage(1)
  }, [keyword, selectedCourse, selectedYear, selectedStatus])
      ...
      <div className="max-w-[95rem] mx-auto flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  setSuccess('Sit-in data refreshed.')
                } catch (e) {
            ...
            >
              {refreshingLive ? 'Refreshing...' : 'Refresh Data'}
            </button>
          </div>
        </div>

        {/* ── Search & Filter Controls ── */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col lg:flex-row gap-4 items-center">
          {/* Search Input Box */}
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full bg-gray-50 border-2 border-gray-100 rounded-xl pl-10 pr-4 py-2.5 text-sm font-semibold text-gray-700 focus:outline-none focus:border-[#3c096c] focus:bg-white transition-all"
            />
          </div>

          {/* Quick Filter Selection Deck */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full lg:w-auto lg:min-w-[34rem]">
            {/* Dynamic Course Option Select */}
            <div className="relative">
              <select
                value={selectedCourse}
                onChange={e => { setSelectedCourse(e.target.value); setSuccess('') }}
                className="w-full appearance-none bg-gray-50 border-2 border-gray-100 rounded-xl pl-4 pr-10 py-2.5 text-xs font-bold text-gray-600 focus:outline-none focus:border-[#3c096c] focus:bg-white transition-all cursor-pointer"
              >
            ...
            <div className="relative">
              <select
                value={selectedYear}
                onChange={e => { setSelectedYear(e.target.value); setSuccess('') }}
                className="w-full appearance-none bg-gray-50 border-2 border-gray-100 rounded-xl pl-4 pr-10 py-2.5 text-xs font-bold text-gray-600 focus:outline-none focus:border-[#3c096c] focus:bg-white transition-all cursor-pointer"
              >
            ...
            {/* Live Check-in / Session Status Select */}
            <div className="relative">
              <select
                value={selectedStatus}
                onChange={e => { setSelectedStatus(e.target.value); setSuccess('') }}
                className="w-full appearance-none bg-gray-50 border-2 border-gray-100 rounded-xl pl-4 pr-10 py-2.5 text-xs font-bold text-gray-600 focus:outline-none focus:border-[#3c096c] focus:bg-white transition-all cursor-pointer"
              >
          ...
          {(keyword || selectedCourse !== 'All' || selectedYear !== 'All' || selectedStatus !== 'All') && (
            <button
              onClick={() => {
                setKeyword('')
              ...
              className="px-4 py-2.5 text-xs font-black text-red-500 hover:text-red-600 bg-red-50 hover:bg-red-100/70 border border-red-100 rounded-xl transition-all w-full lg:w-auto flex-shrink-0"
            >
              Clear Filters
            </button>
          )}
